...
const About = () => {
    ...
    return (
        <div className="flex flex-col mt-8 mb-8 gap-12 w-full px-16">
            ...
            <span className="text-4xl font-medium text-left">{state.abouts.data?.data.name}</span>
            ...
            <div className="flex flex-col border border-black rounded-lg py-6 px-10 gap-6 w-full">
                ...
                <div className="flex flex-col lg:flex-row justify-between w-full gap-4">
                    <AboutItem icon={<Phone className="w-[40px] h-[40px]" />} title={"Телефон оператора:"} aboutItem={state.getAboutInfo?.phoneOperator ?? 'Нет данных'} />
                    <AboutItem icon={<Phone className="w-[40px] h-[40px]" />} title={"Телефон менеджера:"} aboutItem={state.getAboutInfo?.phoneManager ?? 'Нет данных'} />
                    <AboutItem icon={<Mail className="w-[40px] h-[40px]" />} title={"Email для связи:"} aboutItem={state.getAboutInfo?.email ?? 'Нет данных'} />
                    <AboutItem icon={<MapPin className="w-[40px] h-[40px]" />} title={"Почтовый адрес:"} aboutItem={state.getAboutInfo?.address ?? 'Нет данных'} />
                </div>
            </div>
        </div>
    )
